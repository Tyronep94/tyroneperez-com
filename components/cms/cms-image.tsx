...
export function CmsImage({ asset, className, sizes = "100vw", priority = false }: { asset: MediaAsset; className?: string; sizes?: string; priority?: boolean }) {
  ...
  return <img className={className} src={asset.public_url} srcSet={srcSet || undefined} sizes={sizes} width={asset.width ?? undefined} height={asset.height ?? undefined} alt={asset.alt_text ?? ""} loading={priority ? "eager" : "lazy"} fetchPriority={priority ? "high" : "auto"} />;
}
