...
export default async function EditContentPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data }, { data: collections }, { data: memberships }, { data: media }] = await Promise.all([
  ...
  if (!data) notFound();
  return <ContentEditor entry={data as ContentEntry} action={saveContent.bind(null, id)} deleteAction={deleteContent.bind(null,id)} collections={collections ?? []} selectedCollections={(memberships ?? []).map(row => row.collection_id)} media={media ?? []} />;
}
