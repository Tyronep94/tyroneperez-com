...
import { portfolioAudioMedia, type ContentEntry } from "@/types/cms";
...
export default async function DraftPreviewPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const { data } = await supabase.from("content_entries").select("*,cover_asset:media_assets!content_cover_asset_fk(*),audio_asset:media_assets!content_entries_audio_asset_id_fkey(*),audio_artwork_asset:media_assets!content_entries_audio_artwork_asset_id_fkey(*)").eq("kind", "portfolio").eq("slug", slug).maybeSingle();
  if (!data) notFound();
  ...
  const audio = portfolioAudioMedia(item);
  const mediaType = audio || item.kind === "album" || item.kind === "song" ? "audio" : "image";
  return <div className={`${geist.variable} public-site`}>
    ...
    <main id="main-content" className={`project-page project-page--${mediaType} preview-page`}>
      <section className="project-hero"><div className="public-container"><Link href="/portfolio" className="public-text-link project-back">Back to portfolio</Link><div className="project-hero__copy"><p className="public-kicker">{item.category || "Selected work"} · Preview</p><h1>{item.title}</h1>{item.excerpt && <p>{item.excerpt}</p>}</div>{item.audio_artwork_asset || item.cover_asset ? <CmsImage asset={(item.audio_artwork_asset || item.cover_asset)!} className="cms-project-cover" sizes="(max-width: 1320px) calc(100vw - 40px), 1280px" priority /> : !audio && <MediaPlaceholder item={{title:item.title,mediaType,format:"wide",palette:"noir"}} />}{audio && <PortfolioAudioPlayer media={audio} audioAsset={item.audio_asset} className="portfolio-audio-card--detail" />}</div></section>
